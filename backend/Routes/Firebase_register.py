...
def google_auth(email, display_name, photo_url, uId, last_login_at, last_sign_in_at, creation_time, email_verified, isLogin=False, isUId=""):
    ...
    try:
        ...
        #     firebaseUser = firebaseDataStore.collection('users').document(uId).set({
        #         'email': email,
        ...
        #         "user_psql_id": None
        #     })
        #     access_token = create_access_token(identity=uId, expires_delta=timedelta(days=1), additional_claims={"user_id": firebaseUser.uid, "user_email": email, "token_type": "access"})
        #     refresh_token = create_refresh_token(identity=uId, expires_delta=timedelta(days=1), additional_claims={"user_id": firebaseUser.uid, "user_email": email, "token_type": "refresh"})
        #     return jsonify({"msg": "User Registered", 'access_token': access_token, "refresh_token": refresh_token}), 200
        
        checkFirebaseUser = firebaseDataStore.collection('users').document(uId).get()

        if checkFirebaseUser.exists:
            print("User already exists in Firebase")
            access_token = create_access_token(identity=uId, expires_delta=timedelta(days=1), additional_claims={"user_id": uId, "user_email": email, "token_type": "access"})
            refresh_token = create_refresh_token(identity=uId, expires_delta=timedelta(days=1), additional_claims={"user_id": uId, "user_email": email, "token_type": "refresh"})
            return jsonify({"msg": "User Logged in", 'access_token': access_token, "refresh_token": refresh_token}), 200
        else:
